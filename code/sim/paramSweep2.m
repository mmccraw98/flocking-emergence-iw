...
dim_r0s = [2 5 10];
dim_v0s = logspace(-3, 2, 10);
log_axis = logspace(-3, 1, 10);  % for eta, beta, gamma % MAYBE ADD ANOTHER AT 1e0!

eta = 1e-3;  % noise magnitude
beta = 1e-3;  % excluded volume magnitude

num_repeats = 10;  % each simulation will be repeated this many times and the results will be averaged

count = 1;
total = length(dim_r0s) * length(dim_v0s) * length(log_axis);
for i1 = 1:length(dim_r0s)
    for i2 = 1:length(dim_v0s)
        for i3 = 1:length(log_axis)  % gamma
            ...
            gamma = log_axis(i3);  % strength of attraction to chimney

            % check if the path exists to avoid overwriting
            ordername = sprintf('param_sweep_2/eta1e%d_beta1e%d_gamma1e%d_theta%dpi_ro%d_vo1e%d_.csv', log10(eta), log10(beta), log10(gamma), theta / pi, dim_r0, log10(dim_v0));
            if isfile(ordername)
            ...
            parfor ii = 1:num_repeats
                ...
                name = sprintf('param_sweep_2_com_pos/eta1e%d_beta1e%d_gamma1e%d_theta%dpi_ro%d_vo1e%d_run%d_.csv', log10(eta), log10(beta), log10(gamma), theta / pi, dim_r0, log10(dim_v0), ii);
